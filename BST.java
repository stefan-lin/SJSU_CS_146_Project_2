...
public class BST<T extends Comparable<T>> {
  ...
  protected BinaryNode<T> _head = null; // ROOT OF THE BST
  ...
  public BST(T input_value){
    _head = new BinaryNode<>(input_value);
    _num_of_nodes = 1;
  }
   ...
   */
  private void _inorder(BinaryNode<T> root){
    if(root != null){
  ...
  public void print(){
    /*
    int __print_count = 1;
    LinkedList<BinaryNode<T>> __local_queue = new LinkedList<>();
    if(_head == null){
      System.out.println("Empty Tree");
      return;
    }
    BinaryNode<T> __curr_node = _head;
   ...
   */
  private void _print_tree_lines(int node_num, BinaryNode<T> node){
    /**
     * INDENTATION:
  ...
  public boolean search_node(T input_value) {
    ...
    BinaryNode<T> __node_ref = _find_node(this._head, input_value);
   ...
   */
  protected BinaryNode<T> _get_max_node(BinaryNode<T> node){
   ...
   */
  private BinaryNode<T> _find_node(BinaryNode<T> root, T input_value){
    if(root == null){
      // NOT FOUND
      return null;
    }
    int __result = root.get_value().compareTo(input_value);
    if(__result == 0){
      // FOUND VALUE
      return root;
    }
    else if(__result > 0){
      return _find_node(root.get_left(), input_value);
    }
    else{
      return _find_node(root.get_right(), input_value);
    }
  }
   ...
   * @return
   */
  protected int _get_height(BinaryNode<T> node){
  ...
  public T pop(){
    BinaryNode<T> __deleted_root = _head;
